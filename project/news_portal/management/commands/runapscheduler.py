...
from news_portal.models import News, Subscription
...
def my_job():
    ...
    posts = News.objects.filter(publish__gte=last_week)
    categories = set(posts.values_list('post__id', flat=True))
    subscribers = set(Subscription.objects.filter(post__in=categories).values_list('user__email', flat=True))
